...
```{r}
logisitic_regression <- logistic_reg() %>% 
...
logistic_model <- churn_wf %>% 
    add_model(logisitic_regression) %>%
    fit_resamples(
...
```{r}
(logisitic_results <- collect_metrics(logistic_final) %>% 
...
```{r}
(final_results <- rbind(logisitic_results, boosted_results))
```
